...
class Astar
{
  ...
  void c_h(Vertex& v){
    ...
    {
      v.h += (abs(v.stat[i]/mg.w - i/mg.w) + abs(v.stat[i]%mg.h - i%mg.h)) * H_FACTOR;
    }
    //v.h = abs(v.i - mg.goal);
  }
  ...
  void insert2open_list(Vertex& v)
  {
    ...
    open_vertex tn = {&v};
    if (open_list.count(tn)||close_index.count(v.i))
    {
      if(tn.v->g <= cn.v->g + 1)
      {
  ...
  void get_next_cn2()
  {
    ...
    if (open_list.size())
    {
      ...
      //while((cn.v->f == c_f)&&(cn.v->g == c_g)&&open_list.size())
      {
...
public:
  ...
  void run()
  {
    ...
    while(!close_index.count(gn.v->i))
    {
      if(first)
      {
        first = 0;
